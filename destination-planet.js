...
const BODIES = {
  ...
  'titan-image': { url: '/textures/titan.jpg', rim: [1.0, 0.72, 0.3] },
}
...
async function boot() {
  ...
  function applyTexture(id, animate) {
    ...
    loader.load(
      ...
      undefined,
      (err) => console.error('[planet] LOAD FAILED', body.url, err)
    )
    if (animate && !reduce) {
  ...
  const selectedTab =
    document.querySelector('.grid-container--destination [role="tab"][aria-selected="true"]') ||
  ...
  document
    .querySelectorAll('.grid-container--destination [role="tab"][data-image]')
    .forEach((tab) => {
